...
from data.nodes import Kanji, Stroke, CharacterPart, ChineseCharacter
...
def get_component(xml):
    return ChineseCharacter(
        subcomponents=[CharacterPart],

        component_tag=xml.attrib['id'],
...
def get_componentlink(cpn, xml, position):
    return IsWrittenWith(
        begin_node=cpn,
        end_node=ChineseCharacter(subcomponents=[Kanji],
                                  writing=xml.attrib['element']),
        position=position
    )
